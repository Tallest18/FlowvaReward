...
export const StreakCalendar: React.FC<StreakCalendarProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300">
      ...
      <div className="flex justify-between gap-1 mb-4">
        {daysOfWeek.map((dayInfo, idx) => (
          <div key={idx} className="flex flex-col items-center">
            ...
            >
              {/* Day letter INSIDE the circle */}
              <span className={`text-xs font-semibold absolute ${
                dayInfo.hasCheckIn ? 'text-white' : 'text-gray-600'
              }`}>
                {dayInfo.day}
              </span>
              
              {/* Checkmark overlays the day letter when checked in */}
              {dayInfo.hasCheckIn && (
      ...
      <p className="text-xs text-gray-600 mb-4 mt-8 text-center">Check in daily to earn +5 points</p>

      <button
        onClick={onCheckIn}
        disabled={!canCheckInToday || checkingIn}
        className={`mt-10 mb-10 w-full py-3 rounded-full font-semibold transition-all duration-300 flex items-center justify-center space-x-2 ${
